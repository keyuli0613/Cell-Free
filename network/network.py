...
class MultiCellNetwork:
    ...
    def reset(self):
        ...
        self._timer = 0
        self.tau_p = 14
        self.pilot_usage = {i: [] for i in range(self.tau_p)}
        self.previous_pilot_usage = self.pilot_usage
        self.cluster_size = 6
        self.min_cluster = 1
        self.max_cluster = 20
    ...
    def assign_pilot(self, ue):
        for p, ues in self.pilot_usage.items():
            if not ues:
                self.pilot_usage[p].append(ue)
                ue.pilot = p
                return p
        
        bs_id = max(ue.bss, key=lambda b: ue._gains[b.id]).id
        pilot_interference = np.zeros(self.tau_p)
        for p, ues in self.pilot_usage.items():
            pilot_interference[p] = sum([u._gains[bs_id] for u in ues])
        p = np.argmin(pilot_interference)
        self.pilot_usage[p].append(ue)
        ue.pilot = p
        return p
    ...
    def compute_gamma(self):          
        for p, ues in self.pilot_usage.items():
            if self.previous_pilot_usage[p] != ues:
                for i in self.bss.keys():
                    sum_gains = sum([u._gains[i] for u in ues])
                    for u in ues:
                        u._gamma[i] = self.tau_p * u._gains[i] ** 2 / (self.tau_p * sum_gains + 1e-2)

    def compute_delta(self):
        if self.ues:
            for i, b in self.bss.items():
                all_ues = list(self.ues.values())
                all_gains = [ue._gains[i] for ue in all_ues]
                sorted_indices = np.argsort(all_gains)[::-1]
                sorted_ues = np.array(all_ues)[sorted_indices]
                sorted_gains = np.array(all_gains)[sorted_indices]
    ...
    def scan_connections(self):
        ...
        for ue in self.ues.values():
            if len(ue.bss) < ue.cluster_size:
                ue.request_connection()
            if not ue.pilot and ue.bss:
                self.assign_pilot(ue)
    ...
    def set_cluster_size(self, size):
        new_size = self.cluster_size + size
        if self.min_cluster <= new_size <= self.max_cluster:
    ...
    def info_dict(self, include_bs=False):
        ...
        infos = dict(
            ...
            signal_power=sum(ue.signal_power for ue in self.ues.values()) / (self.num_ue + 1e-3),
